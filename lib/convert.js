...
const { loadTypeMetadata } = require("./typeMetadata");
const {
  DEFAULT_BUNDLE_ROOT,
  JITA_44_STATION_ID,
  JITA_SYSTEM_ID,
...
// Category 8 = Charge. Fitting flag range mirrors buildMovedItemState in
// server/src/services/inventory/itemStore.js (includes bays such as DroneBay).
const CHARGE_CATEGORY_ID = 8;
const FITTING_FLAG_MIN = 11;
const FITTING_FLAG_MAX = 132;

// Shape mirrors DEFAULT_MODULE_STATE in itemStore.js, with online: true.
function buildFittedModuleState() {
  return {
    online: true,
    damage: 0,
    charge: 0,
...
function convertAssetsToItems(assets, characterID, namesByItemId, resolver, fallbackStationID, typeTable) {
  const items = [];
  ...
  for (const asset of list) {
    ...
    const categoryID = typeTable.categoryIdOf(typeID);
    const isCharge = categoryID === CHARGE_CATEGORY_ID;
    // ESI is_singleton covers ships, modules, containers, blueprints, etc.
    // Charges are always stored as a stack in EveJS (loaded ammo included),
    // otherwise mergeItemStacks refuses the reload with STACK_REQUIRED.
    const singleton = !isCharge && asset.is_singleton ? 1 : 0;
    ...
    const row = {
      ...
      tq: {
        ...
        // Raw ESI value, kept traceable after the charge stack override.
        is_singleton: asset.is_singleton || false,
      },
    };

    // The importer writes rows verbatim and moduleState is never re-derived,
    // so fitted modules must carry the online state buildMovedItemState would
    // have given them. Charges never get a moduleState.
    if (!isCharge && flagID >= FITTING_FLAG_MIN && flagID <= FITTING_FLAG_MAX) {
      row.moduleState = buildFittedModuleState();
    }
...
function convertCharacter(characterDir, options = {}) {
  ...
  const typeTable = options.typeTable || loadTypeMetadata(options.typeDataPath);
...
function convertDump(dumpRoot, options = {}) {
  ...
  // Fail before any conversion work if the type table is missing/unreadable.
  const typeTable = loadTypeMetadata(options.typeDataPath);
  console.log(`  Item types: ${typeTable.size} from ${typeTable.path}`);

  const characters = [];
  const allRemaps = [];
  for (const dir of characterDirs) {
    console.log(`  Converting ${path.basename(dir)} ...`);
    const entry = convertCharacter(dir, { fallbackStationID, typeTable });
